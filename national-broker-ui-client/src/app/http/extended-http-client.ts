...
export class ExtendedHttpClient {
  ...
  setOptions(options?: IRequestOptions): IRequestOptions {
    ...
    console.log("got Id token: " + this.securityService.getCurrentUser().idToken);
    options.headers = options.headers.append("Authorization", "bearer " + this.securityService.getCurrentUser()?.idToken?.access_token);
    console.log("HTTP Options add header: " + JSON.stringify(options));
    return options;
  }
